    ...
    try {
      ...
      for (i=0;i<data.length;i++){
        ...
        for (j=0;j<campos.length;j++){
          ...
          column = document.createElement("td");
          column.textContent = dataOnlyOne[0].name +" "+ dataOnlyOne[0].lastname; 
...
editHistory.addEventListener("click",()=>{
    ...
    if (editDescription.value != "" && editDate.value != "" && editNote.value != "" && editIdcitizen.value != "")
    {
      console.log("object");
    fetch("http://localhost:3000/history/update/"+idEditDelete,{
        method: "PUT",
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({description: editDescription.value,date:editDate.value,note:editNote.value,id_citizen:editIdcitizen.value})
    });
    Swal.fire({
      position:"center",
      icon: "success",
...
postHistory.addEventListener("click",()=>{
    ...
    if (postDescription.value != "" && postDate.value != "" && postNote.value != "" && postIdcitizen.value != "")
    {
        fetch("http://localhost:3000/history/create/",{
            method: "POST",
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({description: postDescription.value,date: postDate.value,note:postNote.value,id_citizen:postIdcitizen.value})
        });
        Swal.fire({
          position:"center",
          icon: "success",
